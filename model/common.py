...
class ModelLoader:
    ...
    def load_model_data(self, model_name, model_parameters_class, device):
        """Load model data from the saved file."""
        try:
            # Try the new method first (PyTorch 2.3+)
            torch.serialization.add_safe_globals([model_parameters_class])
        except AttributeError:
            # Fall back to the old method for older PyTorch versions
            pass

        model_location = self.model_location(model_name)

        # Add the model directory to sys.path temporarily for unpickling
        import sys
        import os
